<template>
  ...
  >
    ...
    <div :style="stylevalue" class="single_talk">
      <div
      ...
      ></div>
      <div v-for="(talk_item,index) in talk" :key="index" class="total_talk_form">
        <div class="talk_left">
...
export default {
  data() {
    return {
      ...
      cur_height: this.course_img_height * this.talk.length+50,
    };
  },
    mounted() {
    let _this = this;
    let talk = document.getElementById(this.id);
    let oDrag = new Drag(_this,talk, { handle: talk });
  ...
  computed: {
    ...
    stylevalue() {
      let stylevalue_s =
        "width:" + this.cur_width + "px;" + "height:" + this.cur_height + "px;";
      return stylevalue_s;
    },
    img_form() {
      let img_form_s =
